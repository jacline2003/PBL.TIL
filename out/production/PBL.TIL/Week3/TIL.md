...
- Roles 클래스에서는  과제 제출 가능 여부를 직접 판단하지 않고 정책 객체에게 물어보는 방식으로 구현했다.
...
4. 느낀 점
- 처음에는 왜 굳이 Policy를 따로 만들어야 하는지 이해가 잘 안 됐는데, 직접 해보니 역할과 정책을 분리하는 이유를 알게 되었다.
- 특히 "과제 제출 가능 여부"를 객체가 아니라 정책 객체가 판단하도록 만든 구조가 기억에 남는다.
